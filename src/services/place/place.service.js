...
function savePlace(place) {
    //if needed, convert the tags comma seperated string to an array  
  if (!Array.isArray(place.tags)) {
    place.tags = place.tags.split(',').map(item=>item.trim());
  }
  var idx = places.findIndex(currPlace => currPlace.id === place.id);
  //if this is a new place - add id and push to array
  if (idx === -1){ 
...
function generatePlaces() {
  var places = [];
  places.push(generatePlace(1, 'MisterBit','Habonim St. 4, Ramat Gan, Israel',{lat:32.087893,lng:34.803038},['work','study','fun']));
  places.push(generatePlace(2, 'KC Home','Fierberg St. 34, Tel Aviv, Israel',{lat:32.069557,lng:34.777067},['center','family','fun']));
  places.push(generatePlace(3, 'KO Home','Ben Yehuda St. 105, Tel Aviv, Israel',{lat:32.091208,lng:34.774210},['home']));
  return places;
}


function generatePlace(id, name, address, position, tags) {
  return {
    id,
    name,
    address,
